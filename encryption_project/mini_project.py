...
def XOR_DNA(plainDNA, keyDNA):
    plainDNA_number = convert_DNA_to_binary(plainDNA)
    keyDNA_number = convert_DNA_to_binary(keyDNA)
    ...
    for n in range(0, len(encoded_dna_num), 2) :
        encoded_dna += decodeDNA(encoded_dna_num[n] + encoded_dna_num[n + 1])
...
# REMEMBER WATSON CRICK SAYS A PAIRS WITH T & G PAIRS WITH
assert XOR_DNA("ACGT", "CGTA") == "AGAG" 
assert XOR_DNA("AGAG", "CGTA") == "ACGT"
print("PASSED")